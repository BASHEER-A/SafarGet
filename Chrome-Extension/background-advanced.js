...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.type) {
        ...
        case 'get_performance':
            ...
            case 'extractYouTubeQualities':
                try {
                    extractYouTubeQualities(request.url)
                        .then(result => {
                    ...
                    console.error('❌ Error in extractYouTubeQualities:', error);
                    sendResponse({ success: false, error: error.message });
                }
            break;
            ...
            case 'downloadYouTube':
                try {
                    sendYouTubeDownload(request)
                        .then(success => {
                            console.log('📤 YouTube download result:', success);
...
// =================================================
// 🎥 YouTube Quality Extraction (yt-dlp integration)
// =================================================

// استخراج جودات YouTube باستخدام yt-dlp
async function extractYouTubeQualities(url) {
    console.log('🎥 Starting YouTube quality extraction for:', url);
    
    await connectWebSocket();
    
    if (!ws || ws.readyState !== WebSocket.OPEN) {
        console.error('❌ WebSocket not connected for YouTube extraction');
        return { success: false, error: 'Not connected to SafarGet' };
    }
    
    return new Promise((resolve) => {
        const requestId = Date.now().toString();
        console.log('📤 Sending extraction request with ID:', requestId);
        ...
        const messageHandler = (event) => {
            try {
                ...
                console.log('📥 Received WebSocket message:', data.type, 'RequestID:', data.requestId);
                
                // تأكيد بدء الاستخراج
                if (data.type === 'extractionStarted' && data.requestId === requestId) {
                    console.log('✅ YouTube extraction started on server');
                    extractionStarted = true;
                    // تمديد الوقت المسموح به
                    clearTimeout(timeoutId);
                    timeoutId = setTimeout(() => {
                        console.error('⏱️ YouTube extraction timeout after starting');
                        ws.removeEventListener('message', messageHandler);
                        resolve({ success: false, error: 'Extraction timeout - process taking too long' });
                    }, 60000); // 60 ثانية بعد البدء
                }
                
                // نتائج الاستخراج
                if (data.type === 'youtubeQualities' && data.requestId === requestId) {
                    clearTimeout(timeoutId);
                    ws.removeEventListener('message', messageHandler);
                    
                    if (data.error) {
                        console.error('❌ YouTube extraction error:', data.error);
                        resolve({ success: false, error: data.error });
                    } else if (data.qualities && data.qualities.length > 0) {
                        console.log('✅ Received YouTube qualities:', data.qualities.length);
                        resolve({ success: true, qualities: data.qualities });
                    } else {
                        console.log('⚠️ No YouTube qualities found');
                        resolve({ success: true, qualities: [] });
                    }
                }
            } catch (e) {
                console.error('❌ Error parsing YouTube extraction response:', e);
            }
        };
        
        ws.addEventListener('message', messageHandler);
        
        // إرسال طلب استخراج الجودات
        const extractRequest = {
            type: 'extractQualities',
            url: url,
            requestId: requestId
        };
        
        console.log('📤 Sending YouTube extract request:', extractRequest);
        ws.send(JSON.stringify(extractRequest));
        
        // timeout بعد 30 ثانية
        timeoutId = setTimeout(() => {
            console.error('⏱️ YouTube extraction timeout after 30 seconds');
            ws.removeEventListener('message', messageHandler);
            if (!extractionStarted) {
                // Fallback: جودات محددة بـ format IDs للدقة الكاملة
                console.log('🔄 Using fallback YouTube qualities with format IDs');
                const fallbackQualities = [
                    { resolution: '2160p', format: 'mp4', ext: 'mp4', has_video: true, filesize: null, format_id: '313+140/315+140/401+140' },
                    { resolution: '1440p', format: 'mp4', ext: 'mp4', has_video: true, filesize: null, format_id: '271+140/308+140' },
                    { resolution: '1080p', format: 'mp4', ext: 'mp4', has_video: true, filesize: null, format_id: '137+140/299+140' },
                    { resolution: '720p', format: 'mp4', ext: 'mp4', has_video: true, filesize: null, format_id: '136+140/298+140/22' },
                    { resolution: '480p', format: 'mp4', ext: 'mp4', has_video: true, filesize: null, format_id: '135+140/244+140/18' },
                    { resolution: 'Audio Only', format: 'mp3', ext: 'mp3', has_video: false, filesize: null, format_id: 'bestaudio' }
                ];
                resolve({ success: true, qualities: fallbackQualities, fallback: true });
            } else {
                resolve({ success: false, error: 'YouTube extraction taking too long - please try again' });
            }
        }, 30000);
    });
}

// إرسال طلب تحميل YouTube مع جودة محددة
async function sendYouTubeDownload(data) {
    console.log('🎬 Sending YouTube download request:', data);
    
    await connectWebSocket();
    
    if (!ws || ws.readyState !== WebSocket.OPEN) {
        console.error('❌ WebSocket not connected for YouTube download');
        return false;
    }
    ...
    const downloadData = {
        ...
        timestamp: Date.now(),
        downloadId: `yt_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    };
    
    try {
        console.log('📤 Sending YouTube download to WebSocket:', downloadData);
        ws.send(JSON.stringify(downloadData));
        console.log('✅ YouTube download sent successfully');
        return true;
    } catch (error) {
        console.error('❌ Failed to send YouTube download:', error);
        return false;
    }
}
...
console.log('🎬 YouTube yt-dlp integration ready');
